...
def _string_type_symbol(debugger = lldb.debugger):
     _symbol_loaded_address('kclass:kotlin.String', debugger)
...
def kotlin_object_to_cstring(process, object_addr):
    global DEBUG_BUFFER_ADDR, DEBUG_BUFFER_SIZE

    if DEBUG_BUFFER_ADDR == 0 or DEBUG_BUFFER_SIZE == 0:
        DEBUG_BUFFER_ADDR = evaluate("(void *)Konan_DebugBuffer()").unsigned
        DEBUG_BUFFER_SIZE = evaluate("(int)Konan_DebugBufferSize()").unsigned
        log(lambda: "DEBUG_BUFFER_ADDR:{:#x} DEBUG_BUFFER_SIZE:{}".format(DEBUG_BUFFER_ADDR, DEBUG_BUFFER_SIZE))
    ...
    error = lldb.SBError()
    s = process.ReadCStringFromMemory(int(DEBUG_BUFFER_ADDR), int(string_len), error)
...
def select_provider(lldb_val, tip, internal_dict):
    ...
    if soa == 1:
        ret = __FACTORY['string'](lldb_val, tip, internal_dict)
    elif soa == 2:
        ret = __FACTORY['array'](lldb_val, tip, internal_dict)
    else:
        ret = __FACTORY['object'](lldb_val, tip, internal_dict)
...
class KonanHelperProvider(lldb.SBSyntheticValueProvider):
    ...
    def _call_to_string(self):
        # error = lldb.SBError()
        # self._read_string("")
        # result =  self._read_string("extern char const * Konan_DebugToString(void* any); Konan_DebugToString((void*){:#x})".format(self._valobj.unsigned), error)

        # stable_ref = evaluate("(void*)Kotlin_Interop_createStablePointer((void*){:#x})".format(self._valobj.unsigned)).unsigned
        # result = self._read_string("(char*)Konan_DebugToString((void*){:#x})".format(stable_ref), error)
        # evaluate("(void)Kotlin_Interop_disposeStablePointer((void*){:#x})".format(stable_ref))
        
        # if not error.Success():
        #     sys.stderr.write("Konan_DebugToString({:#x}) failed - {}".format(self._valobj.unsigned, error.description))
        #     sys.stderr.write('\n')
        #     raise DebuggerException(error.description)
        # log(lambda : "{:#x}.toString() = {}".format(self._valobj.unsigned, result))
        # TODO: Handle 'None' result
        return kotlin_object_to_cstring(self._process, self._valobj.unsigned)
...
class KonanStringSyntheticProvider(KonanHelperProvider):
    def __init__(self, valobj):
        ...
        s = kotlin_object_to_cstring(self._process, self._valobj.unsigned)
...
class KonanObjectSyntheticProvider(KonanHelperProvider):
    def __init__(self, valobj, tip, internal_dict):
        ...
        log(lambda: "KonanObjectSyntheticProvider::__init__({:#x}) _children_count:{}".format(self._valobj.unsigned, self._children_count))
